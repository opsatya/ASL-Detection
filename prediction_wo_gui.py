...
# Load model
model = load_model(r"D:\Desktop\ASL_Detection\cnn8grps_rad1_model (3).h5")

# Create initial white image
white = np.ones((400, 400, 3), np.uint8) * 255  # Fixed: 3 channels instead of 1
cv2.imwrite(r"D:\Desktop\ASL_Detection\white (1).jpg", white)
...
while True:
    try:
        ...
        if hands:
            ...
            if image.size == 0:
                print("Extracted image is empty, skipping frame")
                continue

            # Create fresh white canvas for each frame
            ...
            if handz:
                ...
                if len(pts) < 21:
                    print("Invalid number of landmarks, skipping frame")
                    continue
                ...
                # Check if white image is valid before showing
                if white.shape[0] > 0 and white.shape[1] > 0:
                    cv2.imshow("2", white)
                else:
                    print("White image has invalid dimensions")
                    continue

                # Prepare image for prediction
                white_for_prediction = white.reshape(1, 400, 400, 3)
                prob = np.array(model.predict(white_for_prediction)[0], dtype='float32')
                ...
                # All your existing condition logic remains the same
                ...
                pl = [ch1, ch2]
                if pl in l:
                    if (pts[0][0] > pts[8][0] and pts[0][0] > pts[4][0] and pts[0][0] > pts[12][0] and pts[0][0] >
                        pts[16][0] and pts[0][0] > pts[20][0]) and pts[5][0] > pts[4][0]:
                        ch1 = 2
                ...
                if pl in l:
                    if pts[6][1] > pts[8][1] and pts[14][1] < pts[16][1] and pts[18][1] < pts[20][1] and pts[0][0] < \
                            pts[8][0] and pts[0][0] < pts[12][0] and pts[0][0] < pts[16][0] and pts[0][0] < pts[20][0]:
                        ch1 = 3
                        print("33333c")
                ...
                if pl in l:
                    if (distance(pts[4], pts[11]) > 50) and (
                            pts[6][1] > pts[8][1] and pts[10][1] < pts[12][1] and pts[14][1] < pts[16][1] and pts[18][
                        1] < pts[20][1]):
                        ch1 = 4
                ...
                if pl in l:
                    if ((pts[6][1] > pts[8][1] and pts[10][1] < pts[12][1] and pts[14][1] < pts[16][1] and pts[18][1] <
                         pts[20][1]) and (pts[2][0] < pts[0][0]) and pts[4][1] > pts[14][1]):
                        ch1 = 1
                        print("111113")
                ...
                if pl in l:
                    if ((pts[6][1] > pts[8][1] and pts[10][1] < pts[12][1] and pts[14][1] < pts[16][1] and pts[18][1] <
                         pts[20][1]) and (pts[2][0] < pts[0][0]) and pts[14][1] < pts[4][1]):
                        ch1 = 1
                        print("1111mmm3")
                ...
                pl = [ch1, ch2]
                if pl in l:
                    if not (pts[0][0] + fg < pts[8][0] and pts[0][0] + fg < pts[12][0] and pts[0][0] + fg < pts[16][
                        0] and pts[0][0] + fg < pts[20][0]) and not (
                            pts[0][0] > pts[8][0] and pts[0][0] > pts[12][0] and pts[0][0] > pts[16][0] and pts[0][0] >
                            pts[20][0]) and distance(pts[4], pts[11]) < 50:
                        ch1 = 1
                        print("111116")
                ...
                if ch1 == 1:
                    ...
                    if ((distance(pts[8], pts[12]) - distance(pts[6], pts[10])) >= 8) and (
                            pts[6][1] > pts[8][1] and pts[10][1] > pts[12][1] and pts[14][1] < pts[16][1] and pts[18][
                        1] < pts[20][1]) and (pts[4][1] > pts[9][1]):
                        ch1 = 'V'
